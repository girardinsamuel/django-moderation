...
def get_change(model1, model2, field, resolve_foreignkeys=False, raw=False):
    try:
    ...
    if raw:
        change = get_raw_change_for_type(
            field.verbose_name,
            (value1, value2),
            field,
        )
...
def get_changes_between_models(
    model1, model2, excludes=None, includes=None, resolve_foreignkeys=False, raw=False
):
    changes = {}
    ...
    for field in model1._meta.fields:
        ...
        if not (isinstance(field, (fields.AutoField,))):
            ...
            changes[name] = get_change(model1, model2, field, resolve_foreignkeys, raw)
...
def get_raw_change_for_type(verbose_name, change, field):
    value1, value2 = change
    change = BaseChange(
        verbose_name,
        field,
